...
const getIconForDomain = (domainName) => {
  switch (domainName) {
    case "Molecular Biology":
      return <Globe color="#084CCB" size={20} />;
    case "Literature":
      return <PenTool color="#084CCB" size={20} />;
    case "History":
      return <BookOpen color="#084CCB" size={20} />;
    default:
  ...
  const fetchChartStats = async () => {
    try {
      ...
      examsSnapshot.forEach((examDoc) => {
        ...
        if (examData.domain) {
          domainCounts[examData.domain] =
            (domainCounts[examData.domain] || 0) + 1;
          totalExamsCount++;
        }
      });
          ...
          {analyticsStats.map((item) => (
            <View key={item.id} style={styles.miniCard}>
              <View style={styles.miniCardHeader}>
                ...
                <View
                  style={[styles.badge, styles[`badge_${item.badge.type}`]]}
                >
                  <Text
                    style={[
                      styles.badgeText,
                      styles[`badgeText_${item.badge.type}`],
                    ]}
                  >
                    {item.badge.text}
                  </Text>

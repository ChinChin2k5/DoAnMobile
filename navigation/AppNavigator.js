...
import Classes_Thi_Sinh from '../Screens_Duy/Classes_Thi_Sinh'; // ← MỚI
...
const TeacherAdminPlaceholder = () => (
  <View style={styles.placeholder}>
    ...
    <Text style={{ fontSize: 13, color: '#718096', marginTop: 8 }}>(Đang phát triển)</Text>
  </View>
);
  ...
  return (
    <Tab.Navigator
      screenOptions={({ route }) => ({
        ...
        tabBarIcon: ({ focused }) => {
          let iconName, label;
          if (route.name === 'Dashboard') { iconName = focused ? 'grid' : 'grid-outline'; label = 'Dashboard'; }
          else if (route.name === 'Classes') { iconName = focused ? 'book' : 'book-outline'; label = 'Classes'; }
          else if (route.name === 'History') { iconName = focused ? 'stats-chart' : 'stats-chart-outline'; label = 'History'; }
          else if (route.name === 'Profile') { iconName = focused ? 'person' : 'person-outline'; label = 'Profile'; }

          return (
            <View style={[styles.tabItem, focused && styles.tabItemActive]}>
              <Ionicons name={iconName} size={22} color={focused ? '#1d4ed8' : '#94a3b8'} />
  ...
  return (
    <Tab.Navigator
      screenOptions={({ route }) => ({
        ...
        tabBarIcon: ({ focused }) => {
          let iconName, label;
          if (route.name === 'AdminHome') { iconName = focused ? 'grid' : 'grid-outline'; label = 'Trang chủ'; }
          else if (route.name === 'AdminClasses') { iconName = focused ? 'book' : 'book-outline'; label = 'Lớp học'; }
          else if (route.name === 'AdminStats') { iconName = focused ? 'stats-chart' : 'stats-chart-outline'; label = 'Thống kê'; }
          else if (route.name === 'AdminProfile') { iconName = focused ? 'person' : 'person-outline'; label = 'Hồ sơ'; }

          return (
            <View style={[styles.tabItem, focused && styles.tabItemActive]}>
              <Ionicons name={iconName} size={22} color={focused ? '#F57C00' : '#94a3b8'} />
              ...
              {route.name === 'AdminProfile' && (
                <TouchableOpacity
                  onPress={async () => {
                    try {
                      await AsyncStorage.multiRemove([
                        'atoza_last_active', 'atoza_session_uid',
                        'pending_login_role', 'userRole', 'userName',
                      ]);
                      await signOut(auth);
                  ...
                  style={{ marginTop: 6 }}
                >
                  <Text style={{ fontSize: 11, color: '#ef4444' }}>Quay về Login</Text>
                </TouchableOpacity>
              )}
            </View>
          );
...
const styles = StyleSheet.create({
  ...
  tabItem: {
    ...
    paddingVertical: 8, width: 85, height: 65, borderRadius: 18,
    marginTop: Platform.OS === 'ios' ? 20 : 0,
  },
  tabItemActive: {
    backgroundColor: '#eff6ff', width: 85, height: 65, borderRadius: 20,
    justifyContent: 'center', alignItems: 'center',
    shadowColor: '#1d4ed8', shadowOpacity: 0.15, shadowRadius: 8, elevation: 4,
    transform: [{ scale: 1.05 }],
  },
  tabLabel: { fontSize: 10, marginTop: 2, color: '#94a3b8', fontWeight: '500' },

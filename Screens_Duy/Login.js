...
export default function Login({ navigation }) {
    ...
    // Track field đang được focus để tự vẽ border thay browser
    const [focusedField, setFocusedField] = useState(null); // 'email' | 'password' | null

    // Inject CSS global 1 lần khi mount trên web:
    // Tắt outline mặc định của browser trên <input> và <textarea>
    // để tránh viền xanh/cam chồng lên border của inputBox
    useEffect(() => {
        if (Platform.OS !== 'web') return;
        const styleId = 'atoza-input-reset';
        if (document.getElementById(styleId)) return; // chỉ inject 1 lần
        const tag = document.createElement('style');
        tag.id = styleId;
        tag.textContent = [
            'input:focus,textarea:focus{outline:none!important;box-shadow:none!important;}',
            'input{caret-color:auto;}', // giữ con trỏ nhập liệu
        ].join('');
    ...
    // Native app: onPressIn/Out hoạt động bình thường
    // Web: handlers này KHÔNG được dùng vì mousedown/touchstart
    //      đã được attach trực tiếp lên DOM trong useEffect bên dưới
    const handleLogoPressIn  = () => { if (Platform.OS !== 'web') startLogoTimer(); };
    const handleLogoPressOut = () => { if (Platform.OS !== 'web') cancelLogoTimer(); };

    // Web: attach native DOM listeners sau khi mount
    // ── Tại sao cần attach trực tiếp lên DOM: ──────────────────────────────
    //   • Mobile Chrome: cần { passive: false } để preventDefault() chặn haptic
    //   • Desktop Chrome/Firefox/Safari: dùng chuột → cần mousedown/mouseup
    //     (touchstart không bao giờ fire trên desktop)
    //   • React synthetic onPressIn/Out bị guard "Platform.OS !== web" ở trên
    //     nên không xử lý web — toàn bộ logic web nằm ở useEffect này
    useEffect(() => {
        if (Platform.OS !== 'web') return;
        const el = logoRef.current;
        if (!el) return;

        // ── MOBILE: touch events với passive:false để preventDefault() hoạt động ──
        const onTouchStart = (e) => { e.preventDefault(); startLogoTimer(); };
        const onTouchEnd   = (e) => { e.preventDefault(); cancelLogoTimer(); };
        const onTouchCancel = ()  => { cancelLogoTimer(); };

        // ── DESKTOP: mouse events (touch không bao giờ fire trên desktop) ──────
        const onMouseDown = (e) => {
            if (e.button !== 0) return; // chỉ xử lý chuột trái
            startLogoTimer();
        };
        const onMouseUp     = () => cancelLogoTimer();
        const onMouseLeave  = () => cancelLogoTimer(); // rời khỏi vùng logo = hủy

        const onContextMenu = (e) => e.preventDefault();

        const passive = { passive: false };
        const active  = { passive: true };   // mouse events không cần passive:false

        el.addEventListener('touchstart',   onTouchStart,  passive);
        el.addEventListener('touchend',     onTouchEnd,    passive);
        el.addEventListener('touchcancel',  onTouchCancel, active);
        el.addEventListener('mousedown',    onMouseDown,   active);
        el.addEventListener('mouseup',      onMouseUp,     active);
        el.addEventListener('mouseleave',   onMouseLeave,  active);
        el.addEventListener('contextmenu',  onContextMenu, passive);

        return () => {
            el.removeEventListener('touchstart',  onTouchStart,  passive);
            el.removeEventListener('touchend',    onTouchEnd,    passive);
            el.removeEventListener('touchcancel', onTouchCancel, active);
            el.removeEventListener('mousedown',   onMouseDown,   active);
            el.removeEventListener('mouseup',     onMouseUp,     active);
            el.removeEventListener('mouseleave',  onMouseLeave,  active);
            el.removeEventListener('contextmenu', onContextMenu, passive);
        };
                    ...
                    {Platform.OS === 'web' ? (
                        ...
                        <div
                            ...
                            style={{
                                ...
                                userSelect: 'none',
                                outlineWidth: 0,   // DOM style — tắt focus ring của div trên web
                                cursor: 'pointer',
                ...
                <Animated.View style={[styles.contentCard, { backgroundColor: cardBgColor }]}>
                    ...
                    <Animated.View style={[styles.inputBox, {
                        ...
                        borderColor: (errors.email && touched.email) ? '#E53935'
                            : focusedField === 'email' ? accentColor
                            : email ? accentColor + '55' : '#E8ECF4',
                    }]}>
                        <Feather name="mail" size={18} style={styles.inputIcon}
                            color={(errors.email && touched.email) ? '#E53935'
                                : focusedField === 'email' ? accentColor
                                : email ? accentColor : '#A0AEC0'} />
                        <TextInput
                            style={styles.input}
                            ...
                            onBlur={() => { setFocusedField(null); setTouched(p => ({ ...p, email: true })); validate(); }}
                    ...
                    <Animated.View style={[styles.inputBox, {
                        ...
                        borderColor: (errors.password && touched.password) ? '#E53935'
                            : focusedField === 'password' ? accentColor
                            : password ? accentColor + '55' : '#E8ECF4',
                    }]}>
                        <Feather name="lock" size={18} style={styles.inputIcon}
                            color={(errors.password && touched.password) ? '#E53935'
                                : focusedField === 'password' ? accentColor
                                : password ? accentColor : '#A0AEC0'} />
                        <TextInput
                            style={styles.input}
                            placeholder="••••••••"
                            placeholderTextColor="#C0C9D8"
                            ...
                            onBlur={() => { setFocusedField(null); setTouched(p => ({ ...p, password: true })); validate(); }}
...
const styles = StyleSheet.create({
    ...
    input: {
        flex: 1, fontSize: 14, color: '#1A202C',
        // outline đã được tắt bằng CSS global inject trong useEffect
        // KHÔNG đặt outline ở đây — RN StyleSheet không nhận prop này
        // và sẽ throw "Please use long-form properties"
    },
